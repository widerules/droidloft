	...
	public:
		wyMagnetsElectricTestLayer() {
			...
			for (int i = 0; i < NMIX; i++) {
				...
				p.y = frand() * h;
				ang = frand() * 3.1415f;
				make_mix(p, ang, (cpFloat) 1.0e7 * cpfpow(-1, i % 2),
						 (cpFloat) 1.0e-3 * cpfpow(-1, i % 2));
			}
		...
		virtual void onTargetSelectorInvoked(wyTargetSelector* ts) {
			switch(ts->getId()) {
				...
				case 4:
					...
					break;
				default:
					wyChipmunkTestLayer::onTargetSelectorInvoked(ts);
					break;
			}
		}
	};
	...
	public:
		...
		void add_box() {
			...
			cpShapeSetTexture2(shape, wyTexture2D::makePNG(RES("R.drawable.blocks")),
							   wyr(wyMath::randMax(2) * size * 2, wyMath::randMax(2) * size * 2, size * 2, size * 2), false);
			shape->e = 1.0f;
			shape->u = 0.0f;
		}
	};
...
///////////////////////////////////////////////////////////////////////////////////////////////
	...
	public:
		wyTexturedOneWayTestLayer() {
			...
			shape->collision_type = 2;
			cpShapeSetTexture2(shape, wyTexture2D::makePNG(RES("R.drawable.blocks")),
							   wyr(wyMath::randMax(2) * radius * 2, wyMath::randMax(2) * radius * 2, radius * 2, radius * 2), false);
